...
import CanvasState, { EditToolModes } from '@/app/create/canvas-state';
...
export default function ToolsMenu({ menuMode }: { menuMode: string }) {
    ...
    const editToolButtons = Array.from(editToolMap).map(([mode, Link]) => {
        return <button onClick={() => {
            ...
            className={`${toolMode == mode ? "bg-pastel-green" : "bg-sky-100"}
                        ${toolMode == mode ? "p-1" : "p-2"}`}>
            <Link />
        </button>
    })
    ...
    return (
        <div className="h-full px-1 w-[12rem]">
            <div className={`${menuMode == "edit" ? "visible" : "hidden"} h-full`}>
                <div className="mt-1 grid grid-cols-4 gap-1 [&>*]:w-full [&>*]:h-full [&>*]:rounded-md [&>*:hover]:bg-pastel-green [&>*:hover]:p-1">
                    {editToolButtons}
                </div>
            ...
            <div className={`${menuMode == "render" ? "visible" : "hidden"} h-full`}>
                ...
                <div className={clsx({ 'h-0': !showLighting, 'h-12': showLighting }, "transition-all duration-300 overflow-hidden m-1 text-xs flex flex-col gap-1")}>
                    <div className="flex gap-1">
                        <input type="range" min="1" max="10" value={ambientLight} step="0.025" className="w-20"
                            onChange={(e) => {
                    ...
                    <div className="flex gap-1">
                        <input type="range" min="1" max="10" value={pointLight} step="0.025" className="w-20"
                            onChange={(e) => {
